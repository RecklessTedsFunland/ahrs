...
# CURRENT_CMAKE_PREFIX_PATH="/opt/ros/groovy"
# CURRENT_WORKSPACE="/Users/kevin/ros_sandbox/ahrs/devel"
...
PYTHONPATH_dir=/lib/python2.7/site-packages
...
SETUP_UTIL=/Users/kevin/ros_sandbox/ahrs/devel/_setup_util.py
...
if [ $IS_RECURSIVE -eq 0 ]; then
  export CMAKE_PREFIX_PATH="$("$SETUP_UTIL" --prefix --name CMAKE_PREFIX_PATH --value "/Users/kevin/ros_sandbox/ahrs/devel:/opt/ros/groovy")$CMAKE_PREFIX_PATH"
fi

# source setup.SHELL from parent workspaces
if [ $IS_RECURSIVE -eq 0 ]; then
  _CATKIN_SETUP_NO_RECURSION=1
  for workspace in $("$SETUP_UTIL" --get-reversed-workspaces --value "/Users/kevin/ros_sandbox/ahrs/devel"); do
...
export CPATH="$("$SETUP_UTIL" --prefix --name CPATH --value "/Users/kevin/ros_sandbox/ahrs/devel$CPATH_dir")$CPATH"
if [ $IS_DARWIN -eq 0 ]; then
  export LD_LIBRARY_PATH="$("$SETUP_UTIL" --prefix --name LD_LIBRARY_PATH --value "/Users/kevin/ros_sandbox/ahrs/devel$LD_LIBRARY_PATH_dir")$LD_LIBRARY_PATH"
else
  export DYLD_LIBRARY_PATH="$("$SETUP_UTIL" --prefix --name DYLD_LIBRARY_PATH --value "/Users/kevin/ros_sandbox/ahrs/devel$LD_LIBRARY_PATH_dir")$DYLD_LIBRARY_PATH"
fi
export PATH="$("$SETUP_UTIL" --prefix --name PATH --value "/Users/kevin/ros_sandbox/ahrs/devel$PATH_dir")$PATH"
export PKG_CONFIG_PATH="$("$SETUP_UTIL" --prefix --name PKG_CONFIG_PATH --value "/Users/kevin/ros_sandbox/ahrs/devel$PKG_CONFIG_PATH_dir")$PKG_CONFIG_PATH"
export PYTHONPATH="$("$SETUP_UTIL" --prefix --name PYTHONPATH --value "/Users/kevin/ros_sandbox/ahrs/devel$PYTHONPATH_dir")$PYTHONPATH"

# run all environment hooks of this workspace
FIND=`which find`
SORT=`which sort`
ENV_HOOKS_GENERIC=$($FIND "/Users/kevin/ros_sandbox/ahrs/devel/etc/catkin/profile.d" -maxdepth 1 -name "*.sh" 2>/dev/null | $SORT)
if [ "$CATKIN_SHELL" != "sh" ]; then
  ENV_HOOKS_SPECIFIC=$($FIND "/Users/kevin/ros_sandbox/ahrs/devel/etc/catkin/profile.d" -maxdepth 1 -name "*.$CATKIN_SHELL" 2>/dev/null | $SORT)
else
  ENV_HOOKS_SPECIFIC=
fi
